...
Respect `upstream-reference-repos.mdc`, `uga-online-secrets-and-ferpa.mdc`, and `uga-ool-github.mdc`.
...
2. **Pre-PR** — Prefer running the same steps as command `uga-online-pre-pr-check`: `git status`, scan diff for secrets and FERPA (student names/IDs), run `npm run build` (or README build step) when applicable.
...
- Author self-check: `uga-online-pre-pr-check`
- Reviewer: `uga-online-pr-review`
