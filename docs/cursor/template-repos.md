...
Each template repo (and this hub) includes:

| Mechanism | Purpose |
|-----------|---------|
| [`.github/COMMIT_TEMPLATE`](../.github/COMMIT_TEMPLATE) | One-line subject + checklist comments |
| `scripts/setup-git-hooks.sh` | **Run once per clone** — sets `core.hooksPath` to `.githooks/` |
| `.githooks/prepare-commit-msg` | Opens template in commit editor |
| `.githooks/commit-msg` | Blocks vague or unfinished messages locally |
| Workflow **Validate commit message** | Fails PRs whose commits break the rules |

Org admins: require the **Validate commit messages** status check on `main` (see [`pr-and-code-review.md`](pr-and-code-review.md)).
