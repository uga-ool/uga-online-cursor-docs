...
Commit when you finish a **logical chunk** of work: a component stub, a fixed demo, a workflow step, or a passing build—not every single line. Run **`pre-commit-review`** in your template repo after staging and before each commit (see that repo’s [`how-to-commit.md`](https://github.com/uga-ool/uga-lit-components/blob/main/docs/cursor/how-to-commit.md) and [`template-repos.md`](template-repos.md)).
